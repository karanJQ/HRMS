  ...
  return (
    <Layout title="Transfer & Posting Management">
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        <div className="card text-center"><p className="text-2xl font-bold text-blue-600">{data.length}</p><p className="text-sm text-slate-400">Total Transfers</p></div>
        <div className="card text-center"><p className="text-2xl font-bold text-yellow-500">{pending}</p><p className="text-sm text-slate-400">Pending Approval</p></div>
        <div className="card text-center"><p className="text-2xl font-bold text-green-600">{completed}</p><p className="text-sm text-slate-400">Completed</p></div>
      </div>
        ...
        <div className="card">
          <div className="table-wrap">
            <table>
              ...
              <tbody>{data.map(t=>(
                <tr key={t.id}>
                  <td><div className="font-medium">{t.emp_name}</div><div className="text-xs text-slate-400">{t.emp_id}</div></td>
                  <td>{t.dept_name}</td>
      ...
      {showForm && (
        <Modal title="Create Transfer Order" onClose={()=>setShowForm(false)}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2"><label className="text-xs text-slate-400 block mb-1">Reason</label><textarea className="input" rows={2} value={form.reason} onChange={e=>setForm({...form,reason:e.target.value})}/></div>
          </div>
